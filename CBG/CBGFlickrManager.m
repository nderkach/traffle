...
+ (CBGFlickrManager *) sharedManagerWithLocation:(CLLocation*)location {
    @synchronized (self) {
        if (sharedManager == nil) {
            sharedManager = [[self alloc] init];
        }
        sharedManager.location = location;
    }
...
//        NSLog(@"Preparing flickr request");
        
        NSString *lat = [NSString stringWithFormat:@"%f", self.location.coordinate.latitude]; //  @"48.8567"; //Paris
        NSString *lon = [NSString stringWithFormat:@"%f", self.location.coordinate.longitude]; // @"2.3508";
        //        NSString *lon = [[NSNumber numberWithDouble:[CBGLocationManager sharedManager].locationBestEffort.coordinate.longitude] stringValue];
...
//        [self.flickrRequest callAPIMethodWithGET:@"flickr.photos.search" arguments:[NSDictionary dictionaryWithObjectsAndKeys:lat, @"lat", lon, @"lon", KFlickrSearchRadiusInMiles, @"radius", @"mi", @"radius_units", KFlickrSsearchTags, @"tags", @"tag_mode", @"any", KFlickrSearchLicense, @"license", @"500", @"per_page", nil]];
    ...
    [self.flickrRequest callAPIMethodWithGET:@"flickr.photos.search" arguments:[NSDictionary dictionaryWithObjectsAndKeys:lat, @"lat", lon, @"lon", KFlickrSsearchTags, @"tags", @"32", @"radius", @"interestingness-desc", @"sort", @"photos", @"media", nil]];
...
//            int high = numberOfPhotos <= 10? numberOfPhotos-1: 9;
            int randomPhotoIndex = [CBGUtil getRandomIntBetweenLow:0 andHigh:(int)numberOfPhotos-1];
        ...
        for (NSDictionary *size in photoSizes) {
            if([[NSString stringWithString:(NSString *)[size objectForKey:@"label"]] isEqualToString:@"Large"]/* || ([[NSString stringWithString:(NSString *)[size objectForKey:@"label"]] isEqualToString:@"Original"] && [[size objectForKey:@"width"] integerValue] > 1024)*/) {
                found = true;
...
//    NSLog(@"Original image height: %d", height);
...
//    NSLog(@"Resized image height: %f", resizedImage.size.height);
    
    UIImage *croppedImage = [resizedImage croppedImage:CGRectMake(cropX, 0, constWidth, constHeight)];
    
//    NSLog(@"Cropped image height: %f", croppedImage.size.height);
